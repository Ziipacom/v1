...
- Tap the discovery menu to open Studio over a vertically paged Discover feed. Swipe up on the exposed media or use the next/previous arrows; the Figma menu and bottom dock remain floating above it. Playable media starts muted, only the current item plays, and opening the editing tray or leaving the screen pauses playback. Feed refreshes retain the current item when available. The feed uses the server's discovery items, never private drafts. The included catalog has one playable film (Sintel, with attribution) and labeled photo/concept samples.
- Choose a Studio category or **Edit** to expand the tray, then select a thumbnail to edit it. Tap the feed title to open the full media viewer and its reactions/comments.
...
- **Record** opens a live camera and microphone preview, not a file picker. Start/stop, camera flip, timer, review, retake and use-clip are implemented with Expo Camera on native and MediaRecorder in browsers (2 minutes / 100 MB). Captures remain local until saved. Camera use requires localhost or trusted HTTPS in browsers; the HTTP LAN preview explains this and links to localhost on the host computer. Rebuild the native development client/APK for the new camera and sharing modules. A camera recording is not a public livestream; live broadcast transport remains unimplemented.
- **Studio → Networks** contains Accounts, Feed, People and Publish. Save/open profiles on all six platforms; signed-in users can sync bounded public Bluesky posts/following. Links never imply OAuth or ownership verification. Other providers currently report adapter/setup requirements rather than simulating feed sync. Sample links stay in memory. No account is pre-connected in the preview.
- In **Networks → Publish**, prepare your own saved original media, then open the native/file share sheet (or download in browsers without file sharing), copy a caption, and finish posting in another app. Overlays, trims and music remain edit metadata and are not rendered into this original export. External delivery is never confirmed merely because a platform or share sheet opened. The People tab offers user-controlled invite sharing; there is no automatic contact import or invitation sending. See [social distribution](../SOCIAL-DISTRIBUTION.md) for integration limits.
